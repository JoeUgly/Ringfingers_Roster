...
slightly diff SB pos at very beginning of game
    ...
    for video_path in all_files_l:
        ...
        try:
            ...
            try:
                ...
                weighted_kbit = kbit_per_frame * vid_frame_total / frame_rate
            except:
            ...
            while vid_frame_count + frame_rate <= vid_frame_total:
                ...
                try:
                    ...
                    vid_frame_count += frame_rate  # Increment to next working frame
                    check = vcap.set(1, vid_frame_count)  # 1 designates CAP_PROP_POS_FRAMES (which frame to read)
                    if not check: raise

                    #if vid_frame_count < 1000: continue
...
def process_frames_f(frame_queue, name_d):
    ...
    while True:
        ...
        try:
            ...
            # Which colors were within range
            upper_confirmed_l = []
            lower_confirmed_l = []

            # Upper and lower SB color detection
            for pos_coords_t in [(upper_confirmed_l, 0.335), (lower_confirmed_l, 0.5486)]:
                current_pos_l = pos_coords_t[0]
                y1_percent = pos_coords_t[1]

                # Grab a row region between name and score
                sb_y1 = round(height * y1_percent)
                sb_y2 = round(sb_y1 + 1)

                sb_na = frame_na[sb_y1:sb_y2, sb_x1:sb_x2]
                sb_trans_l = sb_na[0].transpose().tolist()  # Separate each color channel into its own list


                # Check each color
                for color, color_trans_l in zip(['red', 'green', 'blue'], sb_trans_l):
                    
                    # Check if color is within range
                    if all(140 > xxx > 100 for xxx in color_trans_l):
                        #print('confirmed:', current_pos, color)
                        current_pos_l.append(color)

                    # Check if color is detected above threshhold 
                    elif any(xxx > 100 for xxx in color_trans_l):
                        #print('pop:', current_pos, color)
                        continue  ## Skip on any detected color
                
            # SB is detected if only red or blue is within threshold on upper, and oppposite color is within threshold on lower
            if upper_confirmed_l == ['blue'] and lower_confirmed_l == ['red']:
                print('SB detect. Upper: blue')
            elif upper_confirmed_l == ['red'] and lower_confirmed_l == ['blue']:
                print('SB detect. Upper: red')
            else:
                continue
            ...
            for player, iiii in enumerate(tit_y_percents, 1):
                ...
                for color_trans_l in tit_reg.transpose():
                    if not any(i > 100 for i in color_trans_l):
                        break  ##
                else:
                    title_d[player] = True
            ...
            # Loop for each player
            for player, y_percent in enumerate(party_y_percents, 1):
                y_coord = round(height * y_percent)
